...
def mixture_policy(episodes, timesteps, alpha, epsilon, gamma, init_val, dimension, weights_arr, interval, run=1, save=True):
    ...
    if save == True:
        np.save('mixture_policy/mixture_size{}_locs{}_{}_nsw'.format(env.size, len(env.loc_coords), run), nsw_data)
        np.save('mixture_policy/mixture_size{}_locs{}_{}_total'.format(env.size, len(env.loc_coords), run), total_data)
...
def get_setting(size, num_locs): # stores fixed environment settings (see excel sheet)
    if num_locs == 2:
        ...
        loc_coords = [[0,0],[3,2],[3,0]]
        dest_coords = [[0,4],[3,3],[0,3]]
    elif num_locs == 4:
        loc_coords = [[0,0], [0,5], [3,2], [9,0]]
        dest_coords = [[0,4], [5,0], [3,3], [0,9]]
    elif num_locs == 5:
        loc_coords = [[0,0], [0,5], [3,2], [9,0], [9,9]]
        dest_coords = [[0,4], [5,0], [3,3], [0,9], [9,0]]
...
if __name__ == '__main__':
    ...
    # mixture_policy(args.episodes, args.fuel, args.alpha, args.epsilon, 
    #             args.gamma, args.init_val, args.dimension, [[0.21, 0.79],[1.0, 0.0]])
