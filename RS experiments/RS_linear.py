...
def scalarized_ql(env, init_val, a_space_n, s_space_n, dim, episodes, weights, alpha, gamma, epsilon, run, save=False):
    ...
    Q = np.full([s_space_n+1, a_space_n, dim], init_val, dtype=float)
    ...
    for i in range(1, episodes+1):
        ...
        while not done:
            ...
            next, reward, done, info = env.step(action) # might also have info at left depending on environment
    ...
    if save==True: 
        np.save(file='/Users/oliviafan/Desktop/RG_data/scalarized_ql_run{}_no_gamma_nsw'.format(run),
                arr=nsw_data)
        np.save(file='/Users/oliviafan/Desktop/RG_data/scalarized_ql_run{}_no_gamma_total'.format(run),
                arr=total_data)
    return mean  
...
def simple_grid_search(env, weights_arr, init_val, a_space_n, s_space_n, dim, episodes, alpha, gamma, epsilon, out_name):
    ...
    np.save('all_nsw_{}_dim{}'.format(out_name), all_nsw, dim)
    np.save('all_points_{}_dim{}'.format(out_name), all_points, dim)
    print('------------------FINSIH GRID SEARCH-------------------')
...
if __name__ == '__main__':
    ...
    prs.add_argument("-i", dest="interval", type=float, default=0.01, required=False, help="Interval of weights\n")
    args = prs.parse_args()
    
    fair_env = gym.make('resource-gathering-v0')
    obs = fair_env.reset()  
    fair_env.seed(1122)
    weights_arr = generate_weights(args.dimension, args.interval)

    # weights_arr =[0.33,0.24,0.43]
    weights_arr =[0.45,0.1,0.45]
    for i in range(1, 51):
        scalarized_ql(fair_env, 30, fair_env.action_space.n, fair_env.observation_space, args.dimension, 
                      args.episodes, weights_arr, args.alpha, args.gamma, args.epsilon, i, True)
    # simple_grid_search(fair_env, weights_arr, 0.1, fair_env.action_space.n, fair_env.observation_space,
